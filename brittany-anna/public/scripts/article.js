...
(function (module) {
  ...
  // };
  Article.numWordsAll = () => {
    return Article.all.map(numWords => numWords.body.split(' ').length).reduce((acc, curr) => acc + curr);
  };
  

  Article.allAuthors = () => {
    return Article.all.map((author, i) => Article.all[i].author).reduce((acc, curr) => {
