...
(function(module) {
  ...
  */
    Article.all = articleData.map(articleObject => new Article(articleObject));//this is the source for the articles to load onto the page
  };
  ...
  Article.allAuthors = () => {
    return Article.all
      .map(art => art.author)
      .reduce((uniqNames, author) => {
        if(!uniqNames.includes(author)) {
          uniqNames.push(author);
        }
        return uniqNames;
      }, []);
  };
